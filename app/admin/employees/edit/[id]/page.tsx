...
export default function AdminEditEmployeePage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.employee) {
          ...
          setInterviewDate(emp.interviewDate || '')
          setHireDate(emp.hireDate || '')
          setPhone(emp.phone || '')
          setAddress(emp.address || '')
          setEmployeeNumber(emp.employeeNumber || '')
          setMaritalStatus(emp.maritalStatus || 'single')
          setChildrenCount(emp.childrenCount?.toString() || '0')
          setReligion(emp.religion || '')
          setWorkType(emp.workType || 'full-time')
          setSalary(emp.salary?.toString() || '')
          setPosition(emp.position || '')
          setEmployeeRank(emp.employeeRank || '')
          setPhotoPreview(emp.photoDataUrl || '')
  ...
  const handleSendSMS = async () => {
    ...
    // Fetch current password if not set
    ...
    if (!currentPassword || !currentPassword.trim()) {
      try {
        ...
        if (empRes.ok) {
          ...
          // Password is never returned, so we need to prompt
          currentPassword = prompt('رمز عبور را وارد کنید (یا برای استفاده از رمز قبلی، Cancel بزنید):')
          if (!currentPassword) {
            return alert('رمز عبور الزامی است')
          }
        }
      } catch {
        currentPassword = prompt('رمز عبور را وارد کنید:')
        if (!currentPassword) {
          return alert('رمز عبور الزامی است')
        }
      }
    ...
    setSendingSMS(true)
    try {
      const res = await fetch(`/api/employees/${employeeId}/send-credentials`, {
        method: 'POST',
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        if (data.success) {
          alert('اطلاعات کارمند با موفقیت به‌روزرسانی شد')
          router.push(`/admin/employees/${employeeId}`)
        } else {
          alert(data.error || 'خطا در به‌روزرسانی کارمند')
        }
      } else {
        let errorMessage = 'خطا در به‌روزرسانی کارمند'
        try {
          const err = await res.json()
      ...
      alert(error?.message || 'خطا در اتصال به سرور')
    } finally {
      setSaving(false)
    }
  }
            ...
            <form onSubmit={submit} className="space-y-6">
              ...
              <div className="border-b border-gray-700 pb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-gray-300 mb-2">رمز عبور (خالی بگذارید تا تغییر نکند)</label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        value={password}
                  ...
                  <div className="md:col-span-2">
                    <button
                      ...
                      disabled={sendingSMS || !phone || !username || (!password && !username)}
